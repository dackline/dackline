...
<section>
    <form id="module-form" class="form form-horizontal" method="POST" action="{{ isset($information->id) ? route('admin::informations.update', $information->id) : route('admin::informations.store') }}" x-data="moduleData()" x-init="moduleInit()" x-on:submit="submit()" novalidate>
        @csrf
...
function moduleData(){
    return {
        moduleInit(){
            ...
            @foreach ($locales as $locale)
                new Quill(this.$refs["{{ $locale }}DescriptionEditor"], {
                    modules: {
