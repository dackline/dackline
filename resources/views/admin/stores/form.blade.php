...
<section>
    <div class="row">
        <div class="col-12">
            ...
            @endif
                <div class="card-body">
                    <form id="module-form" class="form form-horizontal" method="POST" action="{{ isset($store->id) ? route('admin::stores.update', $store->id) : route('admin::stores.store') }}" novalidate>
                        @csrf
                        @if(isset($store->id))
                            @method('PUT')
                        @endif
