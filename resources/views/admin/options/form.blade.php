...
<section>
    <form id="module-form" class="form form-horizontal" method="POST" action="{{ isset($option->id) ? route('admin::options.update', $option->id) : route('admin::options.store') }}" x-data="moduleData()" x-init="moduleInit()" x-on:submit="submit()" novalidate>
        @csrf
        @if(isset($option->id))
            @method('PUT')
        @endif
